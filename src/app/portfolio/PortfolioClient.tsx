...
const PortfolioClient: React.FC<{ projects: Project[] }> = ({ projects }) => (
  <section className="pt-28 pb-20 px-4 md:px-8 max-w-7xl mx-auto">
    ...
    <div className="text-center mb-16 lg:mb-20 motion-safe:animate-fade-in">
      ...
      <h1 className="text-3xl md:text-5xl lg:text-6xl font-display text-white mb-6 mt-4">
    ...
    {/* Grille */}
    {projects.length === 0 ? (
      <p className="text-center text-gray-600">Aucun projet pour le moment.</p>
    ) : (
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        {projects.map((project, i) => (
          <div
            key={project.id}
            className="group relative bg-dark-800 border border-dark-700 rounded-2xl overflow-hidden motion-safe:animate-fade-in hover:border-gold-500/20 hover:shadow-xl hover:shadow-gold-500/5 transition-all duration-500"
            style={{ animationDelay: `${i * 0.1}s` }}
          >
            {/* Image */}
            <Link href={project.slug ? `/portfolio/${project.slug}` : '#'} className="block aspect-video relative overflow-hidden bg-dark-700">
              <Image
            ...
            <div className="p-6 space-y-4">
              ...
              <h3 className="text-xl font-display text-white group-hover:text-gold-400 transition-colors">
